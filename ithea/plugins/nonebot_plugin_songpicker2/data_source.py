...
class dataApi():
    ...
    async def search(self, songName: str):
        ...
        if random.choice([True,False]):
            async with httpx.AsyncClient() as client:
                r = await client.get(f"http://music.163.com/api/search/get/web?s={songName}&type=1&offset=0&limit=5")
        else:
            async with httpx.AsyncClient() as client:
                r = await client.post(
    ...
    async def getSongInfo_0(self, songId: int,songName):
        '''
        获取歌曲信息
        '''
        async with httpx.AsyncClient() as client:
            ...
            if r["code"] != 200:
                return None
            return r
    async def getSongInfo_1(self, songId: int,songName):
        '''
        获取歌曲信息
        '''
        async with httpx.AsyncClient() as client:
...
class dataGet(dataApi):
    ...
    async def songIds(self, songName: str, amount=5) -> list:
        ...
        idRange = amount if amount < len(r["result"]["songs"]) else len(r["result"]["songs"])
    ...
    async def songInfo(self, songIdList: list,songName) -> dict:
        '''
        ...
        try:
            ...
            for songId in songIdList:
                songInfo = dict()
                r = await self.api.getSongInfo_0(songId,songName)
                if r is None:
                    raise WrongDataError
            ...
            r = await self.api.getSongInfo_1(songIdList,songName)
            if r is None:
                raise WrongDataError

            for i in range(len(songIdList)):
                songInfo = dict()
                songInfo["songName"] = r["songs"][i]["name"]

                songArtists = list()
                for ars in r["songs"][i]["artists"]:
                    songArtists.append(ars["name"])
                songArtistsStr = "、".join(songArtists)
                songInfo["songArtists"] = songArtistsStr

                songInfo["songAlbum"] = r["songs"][i]["album"]["name"]
                songInfoList.append(songInfo)
                print(songInfo)
                print(songInfoList)
