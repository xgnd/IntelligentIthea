...
from nonebot import on_command, on_message, on_regex, on_startswith, export
...
wb_matcher = on_message(priority=5)
...
wb_del_cmd = on_startswith('删除词条', permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN | PRIVATE_FRIEND, )
...
wb_del_admin = on_startswith('删除全局词条', permission=SUPERUSER, )
...
async def wb_del_all_a(bot: Bot, event: MessageEvent, state: T_State):
    msg = str(event.message)[4:].strip()
    if msg:
        state['is_sure'] = msg

async def wb_del_all_b(bot: Bot, event: MessageEvent, state: T_State):
    msg = str(event.message)[6:].strip()
    if msg:
        state['is_sure'] = msg

wb_del_all_cmd = on_startswith('删除词库', permission=SUPERUSER | GROUP_OWNER | PRIVATE_FRIEND, handlers=[wb_del_all_a])
wb_del_all_admin = on_startswith('删除全局词库', permission=SUPERUSER, handlers=[wb_del_all_b])
wb_del_all_bank = on_startswith('删除全部词库', permission=SUPERUSER, handlers=[wb_del_all_b])
