...
from nonebot.adapters.onebot.v11 import PokeNotifyEvent, MessageEvent, GroupMessageEvent, GroupIncreaseNoticeEvent, MessageSegment, GroupRequestEvent
...
# 用戳一戳触发抽卡
async def _group_poke(bot: Bot, event: PokeNotifyEvent) -> bool:
    if not config.while_season_end:
...
@scheduler.scheduled_job("date", run_date="2022-7-8 22:36:00", id="upgrade")
async def upgrade():
